<template>
	<div>
		<h3>920 REP Ring QoS Configuration</h3>
		<p>
			Set inbound service-policy to match VLAN and outbound
			policy to set EXP.
		</p>
		<div :class="$style.inputFlex">
			...
			<div>
				<h6 :class="$style.h6">REP Segment</h6>
				<input :class="$style.input" v-model="repSegment" />
			</div>
			<div>
				<h6 :class="$style.h6">List of VLANs</h6>
				<input :class="$style.input" v-model="vlanList" />
			</div>
			<div>
				<h6 :class="$style.h6">QoS Group</h6>
				<input :class="$style.input" v-model="qosGroup" />
			</div>
		</div>
		<pre>
      <code>
  class-map match-all REP_<span>{{ repSegment }}</span>_BULK
   match vlan <span>{{ vlanList }}</span>
  !
  policy-map REP_<span>{{ repSegment }}</span>
   class REP_<span>{{ repSegment }}</span>_BULK
    set qos-group <span>{{ qosGroup }}</span>
  !
  ...
  policy-map <span>{{ distOut }}</span>
   ...
   !
  !
  policy-map <span>Q0_OUT</span>
   class class-default
    set qos-group 0
    set mpls experimental imposition 0
   !
  !
  policy-map <span>Q1_OUT</span>
   class class-default
    set qos-group 1
    set mpls experimental imposition 1
   !
  !
  policy-map <span>Q2_OUT</span>
   class class-default
    set qos-group 2
    set mpls experimental imposition 2
   !
  !
  policy-map <span>Q3_OUT</span>
   class class-default
    set qos-group 3
    set mpls experimental imposition 3
   !
  !
  policy-map <span>Q4_OUT</span>
   class class-default
    set qos-group 4
    set mpls experimental imposition 4
   !
  !
  policy-map <span>Q5_OUT</span>
   class class-default
    set qos-group 5
    set mpls experimental imposition 5
   !
  !
  policy-map <span>Q6_OUT</span>
   class class-default
    set qos-group 6
    set mpls experimental imposition 6
   !
  !
  policy-map <span>Q7_OUT</span>
   class class-default
    set qos-group 7
    set mpls experimental imposition 7
   !
  !
  class-map match-any <span>Q67</span>
   match qos-group 7
   match qos-group 6
  !
...
export default {
	setup() {
		...
		const coreOut = 'CORE_OUT';
		const distIn = 'DIST_IN';
		const distOut = 'DIST_OUT';
		const repSegment = '20';
		const vlanList = '1 2 3 4 5';
		const qosGroup = '20';
